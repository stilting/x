...
module.exports = class MessageCleaner extends Plugin {
   startPlugin() {
      ...
      const commandConfig = {
         command: 'clear',
         aliases: this.settings.get('aliases', ['prune', 'purge', 'cl', 'pr']),
         description: 'Clears a certain amount of messages.',
         usage: '{c} (amount) [beforeMessageId]',
         executor: this.clear.bind(this)
      };

      if (Vencord?.Api?.Commands?.registerCommand) {
         Vencord.Api.Commands.registerCommand(commandConfig);
      } else if (Vencord?.api?.commands?.registerCommand) {
         Vencord.api.commands.registerCommand(commandConfig);
      } else {
